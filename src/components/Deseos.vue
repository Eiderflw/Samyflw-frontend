<template>
	...
	>
		<form ref="formAliexpress">
			<div class="flex flex-column gap-1 mt-2">
				<label for="link" class="font-bold block">Url</label>
				<InputText id="link" placeholder="Ingrese el link del producto" v-model="paqueteDeseoAliexpress.link" required />
			</div>
		</form>
		<template #footer>
	...
	<Dialog v-model:visible="abrirModalBorrarDeseo" header="Restablecer Contraseña" :style="{ width: '50rem' }"
      :breakpoints="{ '1199px': '75vw', '575px': '90vw' }" position="top" :modal="true" :draggable="false">
      <h3>
        ¿Esta seguro de borrar {{ confirmarBorrarDeseo.nombre }}?
      </h3>
      <template #footer>
		...
		async cambiarFavorito(idDeseo) {
			this.deseos.forEach((deseo, i) => {
				if (deseo._id == idDeseo) {
					...
					axios
					.post(`${this.API}/tienda/deseos/favorito`, {
						id: this.ID,
						idDeseo
					}, {
						headers: {
