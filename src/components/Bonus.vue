    ...
    <div>
                            ...
                            <template #body="slotProps">
                                <div class="flex flex-wrap gap-2">
                                    <Button icon="pi pi-pencil" outlined rounded severity="warning"
                                        @click="editarBonusDialog(slotProps.data, 'Bonus')" />
                                    <Button icon="pi pi-eye" v-tooltip.top="'¿Quién cumple el bonus?'" outlined rounded severity="info"
                            ...
                            <template #body="slotProps">
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    <Button icon="pi pi-eye" v-tooltip.top="'¿Quién cumple el bonus?'" outlined rounded severity="info"
                                        @click="verQuienCumple(slotProps.data._id)" />
                                    <Button icon="pi pi-trash" outlined rounded severity="danger"
                                        @click="comfirmDelete(slotProps.data._id)" />
                                </div>
                            </template>
                        </Column>
                    </DataTable>
                    ...
                    <div class="flex flex-wrap flex-column gap-3 align-items-center justify-content-center">
                        <form ref="formMezcla" class="md:col-5 xs:col-11 sm:col-10">
                            ...
                            <div class="flex flex-column gap-1 mb-2">
                                ...
                                <MultiSelect v-model="paqueteMezclar.categorias" id="categorias" placeholder="Selecciona las categorías" :options="categorias" />
                            </div>
        ...
        <Dialog v-model:visible="modalBonusCategoria" header="Crear Bonus" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }" position="top" :modal="true" :draggable="false">
            ...
            <template #footer>
                ...
                <Button :label="idEditarBonusCategoria == null ? 'Crear' : 'Actualizar'" @click="crearBonusCategoria" :disabled="btnBonus" severity="success" />
            </template>
        </Dialog>
        ...
        <Dialog v-model:visible="modalVerCumplen" header="Cumplen Bonus" :style="{ width: '54rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }" position="top" :modal="true" :draggable="false">
            <DataTable :value="cumplenBonus" tableStyle="min-width: 100%" sortField="usuario" :sortOrder="1">
                <template #header>
                    <div class="flex flex-wrap align-items-center justify-content-center gap-2">
                        <span class="text-xl text-900 font-bold">{{ cumplenBonus.length }} {{ cumplenBonus.length == 1 ? 'creador cumple el bonus' : 'creadores cumplen el bonus' }}</span>
                    </div>
                </template>
...
export default {
    ...
    methods: {
        ...
        async verQuienCumple(idBonus = null) {
            if (idBonus != null) {
                await axios.get(`${this.API}/usuario/cumplen/bonus/${idBonus}`, this.token).then(resp => {
                    ...
                    switch (error.response.data.statusCode) {
                        case 401:
                            ...
                            break;
                        default:
                            this.$toast.add({ severity: 'error', summary: 'Creadores bonus', detail: 'No se pudieron obtener los creadores', life: 1600 });
        ...
        async crearBonus() {
            ...
            if (valid) {
                ...
                if (this.idEditarBonus == null) {
                    ...
                    //Se está editando
                    await axios.put(`${this.API}/bonus/actualizar/${this.idEditarBonus}`, this.paqueteBonus, this.token).then(resp => {
                        ...
                        this.$toast.add({ severity: resp.data.actualizado ? 'success' : 'error', summary: 'Actualizar bonus', detail: resp.data.message, life: 1600 })
                    }).catch(error => {
                        switch (error.response.data.statusCode) {
                            case 401:
                                //Se le termino la sesión
                                ...
                                break;
                            default:
                                this.$toast.add({ severity: 'error', summary: 'Actualizar bonus', detail: 'Ocurrió un problema actualizando el bonus', life: 1600 });
                                console.log('Error: ', error);
                                break;
                        }
                    });
            ...
            if (this.paqueteBonusCategoria.categorias.length > 0) {
                ...
                if (valid) {
                    ...
                    if (this.idEditarBonusCategoria == null) {
                        await axios.post(`${this.API}/bonus-categoria/crear`, this.paqueteBonusCategoria, this.token).then(resp => {
                            if (resp.data.creado) {
                        ...
                        await axios.put(`${this.API}/bonus-categoria/actualizar/${this.idEditarBonusCategoria}`, this.paqueteBonusCategoria, this.token).then(resp => {
                            if (resp.data.actualizado) {
                                this.getBonosAgrupadosCategoria();
                                this.getBonusMezclados();
                                this.$refs.formBonusCategoria.reset();
                            ...
                            this.$toast.add({ severity: resp.data.actualizado ? 'success' : 'error', summary: 'Actualizar Bonus', detail: resp.data.message, life: 1640 });
                        }).catch(error => {
                            switch (error.response.data.statusCode) {
                                case 401:
                                    //Se le termino la sesión
                                    ...
                                    break;
                                default:
                                    this.$toast.add({ severity: 'error', summary: 'Actualizar bonus', detail: 'No se pudo actualizar el bonus', life: 1600 });
                                    console.log('Error: ', error);
                                    break;
                            }
                        });
        ...
        async getBonosAgrupadosCategoria() {
            await axios.get(`${this.API}/bonus-categoria/agrupados`, this.token).then(resp => {
        ...
        async mezclarCategorias() {
            if (this.paqueteMezclar.categorias.length > 1) {
                this.btnBonus = true;
                await axios.post(`${this.API}/bonus-categoria/mezclar`, this.paqueteMezclar, this.token).then(resp => {
                    if (resp.data.creado) {
                        this.getBonusMezclados();
                        this.categoriasMezcladas = this.paqueteMezclar.categorias;
                        this.activePanel = 5 - this.categoriasMezcladas.length;
                    }
                    this.$toast.add({ severity: resp.data.creado ? 'success' : 'error', summary: 'Mezclar categorías', detail: resp.data.message, life: 1640 });
                }).catch(error => {
                    switch (error.response.data.statusCode) {
                        case 401:
                            //Se le termino la sesión
                ...
                this.$toast.add({ severity: 'error', summary: 'Mezclar categorías', detail: 'Debes seleccionar al menos 2 categorías', life: 1600 });
            }
        },
        async getMezclaCategorias() {
            await axios.get(`${this.API}/bonus-categoria/mezcla-categorias`, this.token).then(resp => {
        ...
        async reiniciarMezclaCategorias() {
            await axios.put(`${this.API}/bonus-categoria/mezcla/reiniciar`, {}, this.token).then(resp => {
                ...
                this.$toast.add({ severity: resp.data.reiniciado ? 'success' : 'error', summary: 'Reiniciar mezcla', detail: resp.data.message, life: 1640 });
            }).catch(error => {
                switch (error.response.data.statusCode) {
                    case 401:
                        //Se le termino la sesión
        ...
        async getBonusMezclados() {
            await axios.get(`${this.API}/bonus-categoria/mezcla-categorias/bonus`, this.token).then(resp => {
        ...
        async getBonusCategorias() {
            await axios.get(`${this.API}/bonus-categoria`, this.token).then(resp => {
        ...
        async deleteBonusBd() {
            ...
            if (this.deleteBonusID != null) {
                await axios.delete(`${this.API}/bonus/${this.deleteBonusID}`, this.token).then(async (resp) => {
                ...
                await axios.delete(`${this.API}/bonus-categoria/${this.idEliminarBonusCat}`, this.token).then(resp => {
                    this.$toast.add({ severity: resp.data.eliminado ? 'success' : 'error', summary: 'Eliminar bonus', detail: resp.data.message, life: 1600 });
...
.usuario>.Bonus>.p-toggleable-content>.p-panel-content>.p-tabview>.p-tabview-panels>.p-tabview-panel>.bonus-usuario>.p-datatable-wrapper {
    height: 650px;
}

.usuario>.Bonus>.p-toggleable-content>.p-panel-content>.p-tabview>.p-tabview-panels>.p-tabview-panel>.bonus-usuario>.p-datatable-wrapper::-webkit-scrollbar {
    width: 8px;
}

.usuario>.Bonus>.p-toggleable-content>.p-panel-content>.p-tabview>.p-tabview-panels>.p-tabview-panel>.bonus-usuario>.p-datatable-wrapper::-webkit-scrollbar-track {
    background: #f1f1f1;
    border-radius: 10px;
}

.usuario>.Bonus>.p-toggleable-content>.p-panel-content>.p-tabview>.p-tabview-panels>.p-tabview-panel>.bonus-usuario>.p-datatable-wrapper::-webkit-scrollbar-thumb {
    background: #888;
    border-radius: 10px;
}

.usuario>.Bonus>.p-toggleable-content>.p-panel-content>.p-tabview>.p-tabview-panels>.p-tabview-panel>.bonus-usuario>.p-datatable-wrapper::-webkit-scrollbar-thumb:hover {
    background: #555;
}
