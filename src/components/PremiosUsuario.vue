<template>
    <Panel class="premios">
        ...
        <CajaSorpresa :mostrarCaja="cajaSorpresa.mostrar" :tipo_premio="cajaSorpresa.tipo_premio" :premio="cajaSorpresa.premio" :descripcion="cajaSorpresa.descripcion" @cerrarCaja="cajaSorpresa.mostrar = false" @reclamandoPremio="() => { paqueteReclamando.tipo_premio != 'SaldoApi' ? cuenta = true : addSaldoByPremio() }" />
    ...
    methods: {
        ...
        async addSaldoByPremio() {
            if (this.paqueteReclamando.tipo_premio == 'SaldoApi') {
                await axios.put(`${this.API}/usuario/saldoByConcurso`, { id_concurso: this.paqueteReclamando.id_concurso, usuario: this.store.getId() }, {
                    headers: {
                        Authorization: `Bearer ${this.store.getToken()}`
                    }
                }).then(response => {
                    ...
                    this.$toast.add({ severity: response.data ? 'success' : 'error', summary: 'Agregar saldo', detail: response.data ? 'Saldo agregado' : 'No se pudo agregar el saldo', life: 1500 });
